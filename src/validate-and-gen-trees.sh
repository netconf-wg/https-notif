...
do
    ...
    response=`yanglint -ii -t config -p ../bin ../bin/ietf-https-notif-transport\@$(date +%Y-%m-%d).yang $name.xml`
    if [ $? -ne 0 ]; then
...
do
    ...
    echo "Validating examples for $name.xml"
    response=`yanglint -ii -t config -p ../bin ../bin/example-custom-module\@$(date +%Y-%m-%d).yang $name.xml`
    if [ $? -ne 0 ]; then
       printf "failed (error code: $?)\n"
       printf "$response\n\n"
       echo
